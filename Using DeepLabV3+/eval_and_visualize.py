...
def postprocess_mask(prob, orig_shape, thr=0.5):
    """
    Accepts:
      - prob : numpy array OR torch tensor containing probabilities (HxW or 1xHxW)
      - orig_shape : (H_orig, W_orig) desired output size (may be invalid)
      - thr : threshold for binarization
    Returns uint8 mask (0/255) resized to orig_shape (or left as-is if orig invalid).
    """
    import numpy as np
    import cv2
    import torch
    ...
    # If prob is a torch tensor on GPU, move to cpu and convert
    if isinstance(prob, torch.Tensor):
        prob = prob.detach().cpu().numpy()

    prob = np.asarray(prob)

    # Handle channel-first (1,H,W) or (H,W,1)
    if prob.ndim == 3 and prob.shape[0] == 1:
        prob = prob[0]
    if prob.ndim == 3 and prob.shape[2] == 1:
        prob = prob[:, :, 0]
    if prob.ndim != 2:
    ...
    if prob.ndim != 2:
        raise ValueError(f"postprocess_mask: cannot interpret prob shape {prob.shape}")

    # Binarize -> 0/255
    mask = (prob >= thr).astype('uint8') * 255

    # Validate orig_shape; fallback if invalid
    try:
        H = int(orig_shape[0])
        W = int(orig_shape[1])
    except Exception:
...
def iou_and_dice(pred_mask, gt_mask):
    ...
    # Binarize
    pred_mask = (pred_mask > 127).astype('uint8')
    gt_mask   = (gt_mask > 127).astype('uint8')

    H, W = pred_mask.shape

    # --- FIX: Resize GT to match prediction size ---
    ...
    union = (p == 1).sum() + (g == 1).sum() - inter

    iou = inter / (union + 1e-6)
    dice = (2 * inter) / ((p == 1).sum() + (g == 1).sum() + 1e-6)
